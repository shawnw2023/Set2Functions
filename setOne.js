...
// CODE HERE
let params = (x, y) => `The first Parameter is ${x}, The second Parameter is ${y}.`

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(params('his', 'hers'))
...
greeting("Shawn", "Wagner", printFullName)
...
function pricesPlusTax(arr, callback){
    let totalCost = []
    for(let i = 0; i < arr.length; i++){
        totalCost.push(arr[i] * 1.2)
    }
    callback(totalCost)
}
function printTotalCost(totalCost){
